    ...
    <div className="container mx-auto px-4 sm:px-6">
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
        ...
        <div>
          ...
          <ul className="space-y-3">
            {["Design Patent", "Utility Patent", "Flowchart", "Flow Diagram", "Mechanical", "Graphs", "Screenshot", "Electrical"].map(service => {
              const categoryId = service.toLowerCase().replace(" patent", "").replace(" & ", "").replace(" ", "");
              return (
                <li key={service}>
                  <Link
                    to={`/samples?category=${categoryId}`}
